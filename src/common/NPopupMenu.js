 ...
 *  @class NPopupMenu
 */
export default class NPopupMenu extends React.Component {
	/*
	 *  @constructs NPopupMenu
	 */
	constructor(props) {
		super(props);
		this.state = {
	...
	*/
	...
	setDisplayTree(tree, prevs, curr, keyPath) {
		...
		if (!curr.children) {
			...
			const itemKey = "menu-leaf-" + curr.id;
			...
			if (shouldDisplay && parentMaxLeaves > relativeIndex ) {
				...
				if (relativeIndex === filteredChildren.length - 1) {
					if (currCustomloadMoreComponent) {
						...
						prevs.push(
							<li key={itemKey}
								className="menu-load-more-container"
								onClick={this.onLoadMoreClick.bind(this, tree, curr, keyPath)}
							>
								<span>Load more</span>
							</li>
		...
		/*the node*/
			...
			const key = "menu-node-" + currLevel + "-" + curr.id;
			...
			if ((!curr.isOpen && !isSearching) || (!curr.isSearchOpen && isSearching)) {
				...
				if (shouldDisplay) {
					...
					if (childrenList.length > 0) {
						openedNode.push(
							<ul key={"menu-children-list" + currLevel}>
								{childrenList}
							</ul>
						);
					}
	...
	render() {
		...
		return (
			<div className="menu-container">
				{headerContent}
				<div className="menu-display-tree-container">
					{bodyContent}
				</div>
			</div>
		);
